...
Cada instalação possui identidade global e jogador local. Perfis isolam conexão, caminho opcional, sessão, logs, downloads e recuperação. Credenciais ficam no Windows Credential Manager. Novos perfis usam `worlds/<worldId>/`; o prefixo vazio continua válido para compatibilidade.

`lock.json` é o manifesto autoritativo. Schema v2 acrescenta nome exibido, pasta canônica, retenção e autor, mantendo leitura do schema v1. Toda mutação usa ETag e compare-and-swap. O ZIP é imutável e enviado antes de mudar `Current`; a versão anterior entra em `History`.
